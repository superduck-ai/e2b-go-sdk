...
// doctest file through an explicit os.Stat("docs/...") assertion.
func TestDocsLocalPagesHaveDedicatedTests(t *testing.T) {
	localDocs := collectMDXRelativePaths(t, "docs")
	referencedDocs := collectReferencedDocsFromDocTests(t)

	missing := missingPaths(localDocs, referencedDocs)
	if len(missing) != 0 {
		t.Fatalf(
			"found %d local docs without a dedicated doctest reference:\n%s",
			len(missing),
			formatPathList(missing),
		)
	}
	...
	if len(extra) != 0 {
		t.Fatalf(
			"found %d doctest references to docs that do not exist:\n%s",
			len(extra),
...
func collectReferencedDocsFromDocTests(t *testing.T) map[string]struct{} {
	t.Helper()
